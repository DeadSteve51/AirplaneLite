...
function import {
    if [ -f "$basedir/Purpur/Purpur-Server/src/main/java/net/minecraft/server/$1.java" ]; then
        echo "ALREADY IMPORTED $1"
        return 0
    fi
    export importedmcdev="$importedmcdev $1"
    file="${1}.java"
    target="$basedir/Purpur/Purpur-Server/src/main/java/$nms/$file"
...
function importLibrary {
    ...
    for file in "$@"; do
        file="$prefix/$file"
        target="$basedir/Purpur/Purpur-Server/src/main/java/$file"
...
for f in $files; do
    ...
    if [ "$?" == "1" ]; then
        if [ ! -f "$basedir/Purpur/Purpur-Server/src/main/java/net/minecraft/server/$f.java" ]; then
            if [ ! -f "$decompiledir/$nms/$f.java" ]; then
                echo "$(bashColor 1 31) ERROR!!! Missing NMS$(bashColor 1 34) $decompiledir/$nms/$f.java $(bashColorReset)";
            else
                import $f
            fi
        fi
